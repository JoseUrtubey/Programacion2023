...
INT DETERMINISTIC BEGIN DECLARE 
	DECLARE cant INT;
	SELECT
...
CREATE PROCEDURE LIST_PROCEDURE(IN CO_NAME VARCHAR(
250), INOUT LIST VARCHAR(500)) BEGIN DECLARE 
	...
	OPEN cursList;
	looplabel: LOOP FETCH cursList INTO coun, f_name, l_name;
	IF finished = 1 THEN LEAVE looplabel;
	END IF;
	IF coun = co_name THEN
	SET
	    list = CONCAT(f_name, ' ', l_name, '; ', list);
	END IF;
	END LOOP looplabel;
	CLOSE cursList;
	END $ 
...
SELECT inventory_in_stock(3000);

-- This one throws 1 (TRUE, because MySQL detects boolean as 0 for FALSE and 1 for TRUE)

SELECT inventory_in_stock(4568);

-- This one throws 0 (FALSE, because MySQL detects boolean as 0 for FALSE and 1 for TRUE)
